...
- CI packages `Rules.zip` (verbatim) and `Rules-minimized.zip` (flow-style `Languages/**/unicode*.yaml` only) via `uv run --project PythonScripts package-rules Rules <output> [--minimize]`.
